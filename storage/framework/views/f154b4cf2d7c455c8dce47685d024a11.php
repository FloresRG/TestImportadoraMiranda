...
<?php $__env->startSection('title', 'Carrito de Ventas Moderno'); ?>
...
<?php $__env->startSection('js'); ?>
    ...
    <script>
        $(document).ready(function() {
            // Variables globales
            let carrito = [];
            let sucursalId = <?php echo e($id ?? 1); ?>;
            let ventaId = <?php echo e($idventa ?? 'null'); ?>;
            let usuario = null;
            let sucursal = null;
            let searchTimeout;
            ...
            `;
                    ...
                    productos.data.forEach(function(item) {
                        ...
                        const stockBadge = item.cantidad > 10 ?
                            ...
                            item.cantidad > 0 ?
                            '<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">Poco Stock</span>' :
                            '<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">Sin Stock</span>';

                        html += `
                    <div class="producto-card rounded-2xl shadow-lg overflow-hidden fade-in">
                        <div class="relative">
                        ...
                        <div class="p-5">
                            <h6 class="text-lg font-bold text-gray-800 mb-2 line-clamp-2">${producto.nombre}</h6>
                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">${producto.descripcion.substring(0, 80)}...</p>
                            <div class="flex justify-between items-center mb-4">
                                <div>
                                    <p class="text-2xl font-bold text-green-600">Bs ${parseFloat(producto.precio_descuento).toFixed(2)}</p>
                                    <p class="text-sm text-gray-500 flex items-center">
                                        <i class="fas fa-cubes mr-1"></i>Stock: ${item.cantidad}
                                    </p>
                                </div>
                            </div>
                            <button class="btn w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-xl py-3 font-semibold agregar-carrito transform transition-all duration-300 hover:scale-105" 
                                    ${item.cantidad <= 0 ? 'disabled' : ''} 
                                    data-producto='${JSON.stringify(producto)}' 
                                    data-cantidad-disponible="${item.cantidad}">
                                <i class="fas fa-cart-plus mr-2"></i>
                                ${item.cantidad <= 0 ? 'Sin Stock' : 'Agregar al Carrito'}
                            </button>
                        </div>
                    </div>
                `;
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u919984931/domains/importadoramiranda.com/resources/views/ventarecojo/pro.blade.php ENDPATH**/ ?>
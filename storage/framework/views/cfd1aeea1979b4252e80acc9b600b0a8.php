...
<?php $__env->startSection('content'); ?>
    ...
    <div class="d-flex flex-wrap gap-3">
        ...
        <?php if($pedidosDuplicados->isNotEmpty()): ?>
            <div class="alert alert-warning alert-custom flex-fill" style="min-width: 300px;">
                <strong>⚠️ Atención:</strong> Hay pedidos duplicados:
                <ul>
                    <?php $__currentLoopData = $pedidosDuplicados; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idPedido => $envios): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li>
                            <span class="pedido-id">ID Pedido: <?php echo e($idPedido); ?></span> –
                            <span class="envio-info">Envíos relacionados: <?php echo e($envios->pluck('id')->implode(', ')); ?></span>
                        </li>
        ...
        <?php if($pedidosConfirmados->isNotEmpty()): ?>
            <div class="alert alert-success alert-custom flex-fill" style="min-width: 300px;">
                <strong>✅ Atención:</strong> Existen pedidos confirmados:
                <ul>
                    <?php $__currentLoopData = $pedidosConfirmados; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idPedido => $envios): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li>
                            <span class="pedido-id">ID Pedido: <?php echo e($idPedido); ?></span> –
                            <span class="envio-info">
                                Estado: <span class="text-success">Confirmado</span> –
                                Envíos relacionados: <?php echo e($envios->pluck('id')->implode(', ')); ?>
    ...
    <div class="container-fluid">
        ...
        <form method="POST" action="<?php echo e(route('envios.storecuaderno')); ?>" id="create-envio-form" class="needs-validation">
            <?php echo csrf_field(); ?>
                    ...
                    <div class="col-md-6 mb-2">
                        <div class="input-group shadow-sm">
                            <input type="text" class="form-control form-control-lg rounded-pill" id="celular"
                                name="celular" placeholder="Buscar por número de celular,id,id_pedido..." required>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                markConfirmedAsSentBtn.addEventListener('click', function() {
                    Swal.fire({
                        title: '¿Estás seguro?',
                        text: "Esto marcará como enviados todos los pedidos confirmados.",
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
                        confirmButtonText: 'Sí, marcar como enviados',
                        cancelButtonText: 'Cancelar'
                    }).then((result) => {
                        if (result.isConfirmed) {

                            // 🔸 Marcar los checkboxes visualmente ANTES de procesar
                            document.querySelectorAll('.enviado-edit').forEach(checkbox => {
                                checkbox.checked = true;
                            });

                            // 🔸 Ahora hacemos el fetch al servidor
                            fetch('<?php echo e(route('envios.markConfirmedAsSent')); ?>', {
                                    method: 'POST',
                                    headers: {
                                ...
                                .then(data => {
                                    if (data.success) {
                                        ...
                                        Swal.fire(
                                            'Error',
                                            'Ocurrió un problema al marcar los pedidos como enviados.',
                                            'error'
                                        );
                                    }
                                })
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                generarPdfBtn.addEventListener('click', function(e) {
                    ...
                    const url = '<?php echo e(route('envios.generarPdfrespaldo')); ?>' + '?' + params.toString();
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                generarPdfBtn.addEventListener('click', function(e) {
                    ...
                    if (valor.length === 0) {
                        // Usando SweetAlert2 para la alerta cuando no se selecciona ningún registro
                        Swal.fire({
                            icon: 'warning',
                            title: '¡Atención!',
                            text: 'Por favor, selecciona al menos un registro.',
                    ...
                    const url = '<?php echo e(route('envios.validarPedidos')); ?>' + '?' + params.toString();
                    ...
                    fetch(url, {
                        ...
                        .then(data => {
                            if (data.error) {
                                // Usando SweetAlert2 para mostrar el error de validación
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error',
                                    text: data.error,
                                ...
                                // Si todo está OK, hacer otra solicitud para generar el PDF
                                localStorage.removeItem('selectedRows');
                                window.open(data.pdfUrl, '_blank');
                                // Recargar la página después de generar el PDF
                                location.reload();
                        ...
                        .catch(error => {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Ocurrió un error al procesar la solicitud. tienes que incluir el pedido',
                                confirmButtonText: 'Aceptar'
                            });
                        });
                });
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                generarFichaBtn.addEventListener('click', function(e) {
                    ...
                    const url = '<?php echo e(route('envios.generarPdfFichasSeleccionadas')); ?>' + '?' + params
                        .toString();
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                generarNotaVentaBtn.addEventListener('click', function(e) {
                    ...
                    const url = '<?php echo e(route('envios.generarNotaVenta')); ?>' + '?' + params.toString();

                    // Abrir en nueva pestaña para generar la nota de venta
                    window.open(url, '_blank');
                });
            });
        </script>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="id_producto_new" name="id_producto">
                                <?php $__currentLoopData = $productos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $producto): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($producto['id']); ?>"><?php echo e($producto['nombre']); ?> (Stock:
                                        <?php echo e($producto['cantidad_sucursal']); ?>)</option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div class="form-group">
...
<?php $__env->startSection('adminlte_js'); ?>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.return-pedido', function(e) {
                ...
                Swal.fire({
                    title: '¿Estás seguro?',
                    text: "Este pedido será devuelto al inventario.",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
    ...
    <script>
        function setPedidoAndEnvioIds(id_pedido, id_envio) {
            ...
            addProductFormNew.action =
                `<?php echo e(route('envio.envioproductos', ['id_pedido' => ':id_pedido', 'id_envio' => ':id_envio'])); ?>`
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                ...
                                $.ajax({
                                    type: 'GET',
                                    url: '<?php echo e(route('envios.getPedidoData')); ?>',
                                    data: {
                                        id_pedido: pedidoId,
                                        id_envio: envioId
                                    },
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Restaurar el estado de los checkboxes al cargar la página
            $(document).ready(function() {
                const savedRows = JSON.parse(localStorage.getItem('selectedRows')) || [];
            ...
            $('#send-semana').on('click', function() {
                ...
                if (selectedRows.length > 0 && idSemana) {
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo e(route('envios.actualizarSemana')); ?>',
                        data: {
            ...
            var table = $('#envios-table').DataTable({
                ...
                stateSave: true,
                ajax: {
                    url: "<?php echo e(route('envios.extra1.data')); ?>",
                    data: function(d) {
      ...
      `;
            ...
            $(document).on('change',
                ...
                function() {
                    ...
                    $.ajax({
                        type: 'PUT',
                        url: '<?php echo e(route('envios.update', 'id')); ?>'.replace('id', id),
                        data: {
            ...
            $(document).on('click', '.delete-envio', function() {
                ...
                if (confirm('¿Estás seguro de eliminar este registro?')) {
                    $.ajax({
                        type: 'DELETE',
                        url: '<?php echo e(route('envios.destroy', 'id')); ?>'.replace('id', id),
                        success: function(response) {
            ...
            $('#delete-selected').on('click', function() {
                if (selectedRows.length > 0) {
                    if (confirm('¿Estás seguro de eliminar estos registros?')) {
                        $.ajax({
                            type: 'DELETE',
                            url: '<?php echo e(route('envios.destroyMultiple')); ?>',
                            data: {
                                ids: selectedRows
                            },
                            success: function(response) {
        ...
        $(document).ready(function() {
            ...
            $(document).on('input', '#id_pedido_search', function() {
                ...
                if (searchQuery.length >
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '<?php echo e(route('envios.searchPedido')); ?>', // Ruta al controlador de búsqueda
                        data: {
            ...
            $(document).on('click', '.search-result', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo e(route('envio.store-product', ['id_pedido' => ':id_pedido', 'id_envio' => ':id_envio'])); ?>'
                        .replace(':id_pedido', id_pedido)
                        .replace(':id_envio', id),
                    success: function(response) {
                ...
                $.ajax({
                    type: 'GET',
                    url: '<?php echo e(route('envios.getPedidoData')); ?>', // Ruta al controlador para obtener los datos del pedido
                    data: {
                        id_pedido: id_pedido
                    },
                    success: function(data) {
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u919984931/domains/importadoramiranda.com/resources/views/envioscuaderno/extra1.blade.php ENDPATH**/ ?>
...
<?php $__env->startSection('title', 'Stock de Sucursales'); ?>
    ...
    <div class="container">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <form method="GET" action="<?php echo e(route('report.stock')); ?>">
                    <div class="form-row">
                        <div class="col-md-3">
                            ...
                            <select name="id_categoria" id="categoria" class="form-control">
                                ...
                                <?php $__currentLoopData = $categorias; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $categoria): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($categoria->id); ?>"
                                        <?php echo e(request('id_categoria') == $categoria->id ? 'selected' : ''); ?>>
                ...
                <div class="mb-3 d-flex flex-wrap gap-2">
                    <a href="<?php echo e(route('report.stocklog')); ?>" class="btn btn-outline-info">
                        <i class="fas fa-history mr-1"></i> Cambios de Stock
                    </a>

                    <a href="<?php echo e(route('auditorias.inventario')); ?>" class="btn btn-outline-warning">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#products-report-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '<?php echo e(route('report.stock')); ?>',
                    data: function(d) {
                    ...
                    <?php $__currentLoopData = $sucursales; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sucursalId): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        {
                            ...
                            render: function(data, type, row) {
                                return `<input type="number" class="form-control form-control-sm stock-input" 
                                            data-product-id="${row.id}" 
                                            data-sucursal-id="<?php echo e($sucursalId); ?>" 
                                            value="${data || 0}" />`;
                            },
                            className: 'text-center'
                        },
            ...
            $('#products-report-table').on('change', '.stock-input', function() {
                ...
                $.ajax({
                    url: '<?php echo e(route('report.updateStock')); ?>',
                    method: 'POST',
                    data: {
            ...
            $('#products-report-table').on('change', '.stock-almacen-input', function() {
                ...
                $.ajax({
                    url: '<?php echo e(route('report.updateAlmacenStock')); ?>',
                    method: 'POST',
                    data: {
                        product_id: productId,
                        new_value: newValue,
            ...
            $('#auditoria_sucursal_id').on('change', function() {
                ...
                if (sucursalId && productoId) {
                    $.ajax({
                        url: '<?php echo e(route('inventario.stock_actual')); ?>',
                        method: 'GET',
                        data: {
            ...
            $('#form-auditoria').on('submit', function(e) {
                ...
                $.ajax({
                    url: '<?php echo e(route('auditoria.detalle.store')); ?>', // Ruta que debes crear
                    method: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.success) {
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u919984931/domains/importadoramiranda.com/resources/views/productos/stockedit.blade.php ENDPATH**/ ?>
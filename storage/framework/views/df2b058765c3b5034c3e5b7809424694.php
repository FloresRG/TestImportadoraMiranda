...
<?php $__env->startSection('content_header'); ?>
    <h1 class="text-center">Agenda de notas</h1>
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css" rel="stylesheet">
    ...
    <style>
        ...
        .fc-header-toolbar .fc-button {
            background: linear-gradient(135deg, #460e57, #8d177b);
            color: #e6f0ff;
        ...
        <div class="row">
            <div class="col-md-12 d-flex justify-content-between">
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('roles.index')): ?>
                    <!-- Botón Agenda Jefa -->
                    <a href="<?php echo e(route('notasjefa.index')); ?>" class="btn btn-gradient-primary btn-lg">
                        <i class="fas fa-calendar"></i> Agenda Jefa
                    </a>
                <?php endif; ?>
            </div>
        </div>
                ...
                <div class="modal-body">
                    ...
                    <form id="formCrearNota">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="fechaNota">Fecha y Hora</label>
                                    <input type="datetime-local" class="form-control" id="fechaNota" name="fecha" required>
                                </div>
                            </div>
                        </div>
                    </form>
    ...
    <div class="modal fade" id="editarNotaModal" tabindex="-1" role="dialog" aria-labelledby="editarNotaModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editarNotaModalLabel">Editar asd</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    ...
                    <form id="formEditarNota">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <label for="fechaEditarNota">Fecha y Hora</label>
                                    <input type="datetime-local" class="form-control" id="fechaEditarNota" name="fecha"
                                        required readonly>
                                </div>
                            </div>
                        </div>
    ...
    <!-- Botón Cerrar -->
    <button type="button" class="btn btn-outline-secondary" data-bs-toggle="tooltip" title="Cerrar" data-dismiss="modal">
        <i class="bi bi-x-circle-fill"></i>
    </button>

    <!-- Botón Eliminar -->
    <button type="button" class="btn btn-outline-danger" id="eliminarNotaModal" data-bs-toggle="tooltip" title="Eliminar Nota">
        <i class="bi bi-trash-fill"></i>
    </button>

    <!-- Botón Imprimir -->
    <button type="button" class="btn btn-outline-info" id="imprimirNotaModal" data-bs-toggle="tooltip" title="Imprimir Nota">
        <i class="bi bi-printer-fill"></i>
    </button>

    <!-- Botón Guardar -->
    <button type="submit" class="btn btn-outline-primary" id="guardarCambiosNota" data-bs-toggle="tooltip" title="Guardar Cambios">
...
<?php $__env->startSection('js'); ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            let eventos = <?php echo json_encode($events, 15, 512) ?>; // Pasamos los eventos desde PHP a JavaScript
            ...
            let calendar = $('#calendar').fullCalendar({
                ...
                eventClick: function(event, jsEvent, view) {
                    ...
                    $('#fechaEditarNota').val(moment(event.start).format('YYYY-MM-DDTHH:mm:ss'));

                    console.log("Datos recuperados del evento:", {
                        title: event.title,
            ...
            $('#guardarNota').click(function() {
                ...
                let fechaHoraActual = moment().format('YYYY-MM-DD HH:mm:ss'); // Obtener fecha y hora actual

                // Validar que los campos no estén vacíos
                if (titulo && contenido && color) {
                    // Enviar la solicitud AJAX para crear la nota
                    $.ajax({
                        url: '<?php echo e(route('notas.store')); ?>', // Ruta definida en el archivo de rutas
                        type: 'POST',
                        data: {
                            titulo: titulo,
                            nota: contenido,
                            fecha: fechaHoraActual, // Enviar fecha y hora actual
                            color: color, // Enviar el color seleccionado
                            _token: '<?php echo e(csrf_token()); ?>' // Asegurarnos de incluir el token CSRF
                        },
            ...
            $('#guardarCambiosNota').click(function() {
                ...
                if (titulo && contenido && color) {
                    ...
                    $.ajax({
                        url: '<?php echo e(route('notas.update', ':id')); ?>'.replace(':id',
                        ...
                        data: {
                            ...
                            nota: contenido,
                            color: color,
                            _token: '<?php echo e(csrf_token()); ?>' // Incluir el token CSRF
                        },
                        beforeSend: function() {
                        ...
                        success: function(response) {
                            // Mostrar un mensaje de éxito with SweetAlert2
                        ...
                        error: function(status) {
                            console.log("Error en la solicitud:", status);
            ...
            $('#eliminarNotaModal').click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '<?php echo e(route('notas.destroy', ':id')); ?>'.replace(':id',
                                eventId), // Ruta para eliminar la nota
                            type: 'DELETE',
                            data: {
                                _token: '<?php echo e(csrf_token()); ?>', // Incluir token CSRF
                            },
                            success: function(response) {
            ...
            // Lógica para imprimir la nota
            $('#imprimirNotaModal').click(function() {
                let eventId = $('#editarNotaModal').data('event-id');
                window.location.href = '<?php echo e(route('notas.imprimir', ':id')); ?>'.replace(':id', eventId);
            });
        });
    </script>
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u919984931/domains/importadoramiranda.com/resources/views/notas/index.blade.php ENDPATH**/ ?>
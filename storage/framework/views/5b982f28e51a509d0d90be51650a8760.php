...
<style>
    .code-header {
    white-space: nowrap;
    padding: 10px 5px; /* Invertimos el padding para dar más espacio vertical */
    /* min-width: 40px; */
    text-align: center;
    writing-mode: vertical-lr; /* Opción moderna */
    transform: rotate(0deg); /* Rotación estándar */
            ...
            <form id="filters-form" class="mb-3">
                ...
                <div class="form-row align-items-center">
                    ...
                    <div class="col mb-2">
                        ...
                        <select id="filters_select" name="filters" class="col mb-2 hidden-checkbox">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addProductFormNew" action="" method="POST">
                        ...
                        <input id="envioIdNew" name="envioId" class="hidden-checkbox"> <!-- Campo oculto para el id de envio -->
...
$.ajax({
    ...
    success: function(data) {
        console.log('Datos actualizados del pedido recibidos:', data);

        if (data) {
            $('#productos_' + envioId).text(data.productos || 'Sin productos');
            $('#cantidad_productos_' + envioId).text(data.cantidad_productos || '0');
            $('#monto_deposito_' + envioId).text(data.monto_deposito || '0.00');
        }

        // Limpiar formulario
        $('#id_producto_new').val(null).trigger('change');
        $('#cantidad_new').val('');
        $('#precio_new').val('');
        $('#total_new').val('');

        // Recargar tabla si deseas sincronizar backend
        $('#envios-table').DataTable().ajax.reload(null, false);
    },
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('#envios-table').DataTable({
                ...
                stateSave: true,
                ajax: {
                    url: "<?php echo e(route('envios.dataTablePendientes')); ?>",
                    data: function(d) {
                ...
                columns: [
                    ...
                    //     render: function(data, type, row) {
                    //         return '<input type="text" class="form-control id-edit" value="' +
                    //             data + '" data-id="' + row.id + '">';
                    //     }
                    // },
                    {
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            const productos = data ? (typeof data === 'string' ? data.split(',') : data) : [];
                            ...
                            const cantidadHtml = `<span id="cantidad_productos_${row.id}" class="texto-cantidad">${row.cantidad_productos || 'N/A'}</span>`;
                            const montoHtml = `<span id="monto_deposito_${row.id}" class="texto-monto">${row.monto_deposito || 'N/A'}</span>`;
  ...
  render: function(data, type, row) {
    ...
    `;
                            ...
                            buttonHTML += `
    ${row.id_pedido ? 
        `<a href="/orden/cuaderno/lppendientes/edit/${row.id}/${row.id_pedido}" class="btn btn-light btn-sm" style="color: #007bff; border-color: #007bff; background-color: #f0f8ff;">
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u919984931/domains/importadoramiranda.com/resources/views/envioscuaderno/indexlapazpendiente.blade.php ENDPATH**/ ?>
<script>
        ...
        Alpine.data("productosApp", (sucursalId) => ({
            ...
            async loadProductos(page = 1) {
                ...
                try {
                    ...
                    document.getElementById("productos-list").innerHTML =
                        '<div class="col-12"><div class="text-center py-5"><div class="bg-light rounded-3 d-inline-flex p-4 mb-3"><i class="fas fa-exclamation-triangle fa-2x text-warning"></i></div><p class="text-muted mb-0" style="font-size: 1.25rem;">Error al cargar productos</p></div></div>';
                }
            },
            ...
            async renderProductos(productos) {
                ...
                if (productos.length === 0) {
                    html =
                        '<div class="col-12"><div class="text-center py-5"><div class="bg-light rounded-3 d-inline-flex p-4 mb-3"><i class="fas fa-inbox fa-2x text-muted"></i></div><p class="text-muted mb-0" style="font-size: 1.25rem;">No hay productos en esta sucursal.</p></div></div>';
                }
            ...
            renderPagination(productos) {
                ...
                for (let i = 1; i <= productos.last_page; i++) {
                    paginationHtml +=
                        `<li class="page-item ${i === productos.current_page ? 'active' : ''}"><a href="#" class="page-link" data-page="${i}">${i}</a></li>`;
                }
                paginationHtml += '</ul>';
        ...
        document.querySelectorAll('input[name="tipo_pago"]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                ...
                if (this.value === "Efectivo") {
                    if (montoPagadoLabel) montoPagadoLabel.textContent =
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (document.getElementById('usar_vendedor_no')) document.getElementById('usar_vendedor_no')
            .addEventListener('change', aplicarUsoVendedor);
        if (document.getElementById('usar_vendedor_si')) document.getElementById('usar_vendedor_si')
            .addEventListener('change', aplicarUsoVendedor);
        ...
        if (vendedorInput) {
            vendedorInput.addEventListener('input', function() {
                ...
                const optionSeleccionada = Array.from(document.getElementById('sugerencias_vendedores')
                    .options).find(option => option.value === nombreSeleccionado);
                if (optionSeleccionada) {
                    ...
                    if (document.getElementById('usar_vendedor_si') && document.getElementById(
                            'usar_vendedor_si').checked) {
                        idUserInput.value = '';
                    } else {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (hiddenInputVendedor && hiddenInputVendedor.value === 'no') {
            if (switchCheckboxVendedor) switchCheckboxVendedor.checked =
                false; // Asegura que el switch esté visualmente apagado
        } else {
            if (switchCheckboxVendedor) switchCheckboxVendedor.checked =
        ...
        if (vendedorSearchInput) {
            vendedorSearchInput.addEventListener('input', function() {
                ...
                const optionSeleccionada = Array.from(document.getElementById('sugerencias_vendedores')
                    .options).find(option => option.value === nombreSeleccionado);
                if (optionSeleccionada && switchCheckboxVendedor && switchCheckboxVendedor
            ...
            vendedorSearchInput.addEventListener('blur', function() {
                if (switchCheckboxVendedor && switchCheckboxVendedor.checked) {
                    const nombreSeleccionado = vendedorSearchInput.value;
                    const optionSeleccionada = Array.from(document.getElementById(
                        'sugerencias_vendedores').options).find(option => option.value ===
                        nombreSeleccionado);
                    if (!optionSeleccionada) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const switchCheckboxDesc = document.getElementById('descuento_switch');
        const hiddenInputDesc = document.getElementById('descuento_hidden');
        const descuentoFields = document.getElementById('descuento-fields');

        function syncDescuentoToggle() {
            if (switchCheckboxDesc && switchCheckboxDesc.checked) {
                descuentoFields.style.display = 'block';
                hiddenInputDesc.value = 'si';
            } else {
                descuentoFields.style.display = 'none';
                hiddenInputDesc.value = 'no';
            }
        }

        syncDescuentoToggle();

        if (switchCheckboxDesc) {
            switchCheckboxDesc.addEventListener('change', syncDescuentoToggle);
        }
    });
            ...
            input.addEventListener('blur', function() {
                ...
                if (this.classList.contains('precio-input')) {
                    ...
                    if (nuevaCantidad > 0) {
                        ...
                        if (carrito[index].stockSucursal && nuevaCantidad > carrito[index]
                            .stockSucursal) {
                            Swal.fire({
                                title: 'Stock Insuficiente',
                                html: `
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            const url = '<?php echo e(route('nota.pdf')); ?>?nombre_cliente=' + encodeURIComponent(
                                ...
                                encodeURIComponent(JSON.stringify(productos)) + '&descuento=' +
                                encodeURIComponent(descuentoInput.value || '0') + '&pagado=' +
                                encodeURIComponent(pagadoInput.value || '0') + '&pagadoqr=' +
                                encodeURIComponent(pagadoqrInput.value || '0') + '&cambio=' +
                                encodeURIComponent(cambioInput.value || '0') + '&tipo_pago=' +
                                encodeURIComponent(tipoPago) + '&garantia=' + encodeURIComponent(
